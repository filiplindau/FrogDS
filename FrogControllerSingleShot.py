...
class FrogController(object):
    def __init__(self, camera_name, start=False):
        ...
        self.scan_params["time_res"] = 1.5e-15
        self.scan_params["spectral_res"] = -0.0585e-9
        self.scan_params["central_wavelength"] = 388e-9
...
class Scan(object):
    def __init__(self, controller, scan_attr_name, scan_dev_name, start_pos, stop_pos, step,
        ...
        self.controller = controller    # type: FrogControllerSingleShot.FrogController
...
class FrogAnalyse(object):
    ...
    def load_data(self):
        ...
        dt = self.controller.scan_params["time_res"]
        t = np.arange(image.shape[0]) * dt
        w = l0 + (np.arange(image.shape[1]) - image.shape[1]/2) * dl
    ...
    def preprocess(self):
        ...
        self.logger.debug("Bad data points: {0}".format(np.isnan(self.scan_raw_data).sum()))
        if self.controller.analyse_params["background_subtract"] is True:
            ...
            self.logger.debug("Background level subtracted")
        else:
        ...
        self.controller.ana_proc_data = proc_thr
...
if __name__ == "__main__":
    ...
    fc = FrogController("gunlaser/cameras/spectrometer_camera")
    # time.sleep(0)
